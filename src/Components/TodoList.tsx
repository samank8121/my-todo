...
export default function TodoList(props: { data: TodoType[] }) {
    const { data } = props;
        ...
        <div className="flex flex-col mx-6">
            {
                todos.map(t => {
                    return (<div id={t.id.toString()} className="flex items-center justify-center w-auto mt-2 border-2 border-gray-500 rounded">
                        <input id={`done_${t.id}`} name={`${t.id}`} type="checkbox" className="w-10 text-center"
                            checked={t.isChecked} onChange={handleDoneInputChange}></input>
                        ...
                        <input id={`btn_i_${t.id}`} type="button" readOnly className="p-3 text-gray-800 bg-gray-100" value="E"
                            onClick={() => editTodo(t)}></input>
                        <input id={`btn_d_${t.id}`} type="button" readOnly className="p-3 text-gray-800 bg-gray-100" value="D"
